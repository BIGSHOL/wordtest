...
export const ChoiceButton = memo(function ChoiceButton({ index, text, state, onClick }: ChoiceButtonProps) {
  ...
  return (
    <button
      onClick={onClick}
      disabled={isDisabled}
      className="flex items-center gap-3 w-full min-h-14 md:min-h-[60px] px-5 py-3 rounded-2xl transition-all duration-200"
      style={{
        background: s.bg,
    ...
    >
      ...
      <span
        className="text-[15px] md:text-[16px] leading-snug text-left"
        style={{
          color: s.textColor,
