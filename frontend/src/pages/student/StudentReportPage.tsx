/**
 * Student-facing read-only mastery report page.
 * Same content as MasteryReportPage but without TeacherLayout sidebar.
 * Opened as popup from TestStartPage "결과 보기" button.
 */
...
export function StudentReportPage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#F8F8F6] flex items-center justify-center">
        <div className="w-8 h-8 border-4 border-[#2D9CAE] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  if (!report) {
    return (
      <div className="min-h-screen bg-[#F8F8F6] flex items-center justify-center">
        <div className="bg-white rounded-2xl border border-[#E8E8E6] p-12 text-center">
          <p className="text-[#7A7A7A]">리포트를 불러올 수 없습니다.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-[794px] mx-auto space-y-6 py-6 print-report">
        ...
        <div className="flex items-center justify-between print-hidden">
          ...
          <button
            ...
            className="h-9 px-[14px] rounded-lg bg-white border border-[#E8E8E6] text-[#7A7A7A] text-sm font-semibold flex items-center gap-2 hover:bg-[#F5F4F1] transition-colors"
          >
        ...
        <div className="bg-white shadow-[0_1px_8px_rgba(0,0,0,0.08)] p-[30px] space-y-4 print-report-card">
          {/* 1. Header */}
              ...
              <table className="border-collapse border border-[#D0D0D0] text-[10px]">
                <tbody>
                  <tr>
                    ...
                    <td className="px-2 py-1 text-[#0D0D0D] border-r border-[#D0D0D0] whitespace-nowrap min-w-[60px]">{report.student_name || '-'}</td>
                    <td className="bg-[#F5F5F5] px-2 py-1 font-semibold text-[#333] border-r border-[#D0D0D0] whitespace-nowrap w-[50px]">학교</td>
                    <td className="px-2 py-1 text-[#0D0D0D] whitespace-nowrap min-w-[50px]">{report.student_school || '-'}</td>
                  </tr>
                  <tr className="border-t border-[#D0D0D0]">
                    ...
                    <td className="px-2 py-1 text-[#0D0D0D] border-r border-[#D0D0D0] whitespace-nowrap">{report.student_grade || '-'}</td>
          ...
          {/* 2. Overview row */}
          {isListening ? (
            /* Listening test: simplified overview */
            <CollapsibleSection title="리스닝 테스트 결과">
