...
test.describe('Teacher Dashboard', () => {
  ...
  test('teacher can view student test results', async ({ page }) => {
    ...
    // Click on student results (button text is '보기')
    await page.getByRole('button', { name: '보기' }).first().click();
    await expect(page).toHaveURL(/\/students\/.*\/(results|mastery)/, { timeout: 10_000 });

    // Should see test result details
    await expect(page.locator('text=정답')).toBeVisible({ timeout: 5_000 });
  });
});
